...
for change in sys.argv[1:]:
    ...
    while 0 != os.system('cd %s ; git fetch http://yauniks.dynvpn.de/%s refs/changes/%s/%s/%s' % (project, data['project'], junk, number, patch_count + 1)):
        patch_count = patch_count + 1

    while 0 == os.system('cd %s ; git fetch http://yauniks.dynvpn.de/%s refs/changes/%s/%s/%s' % (project, data['project'], junk, number, patch_count + 1)):
        patch_count = patch_count + 1

    os.system('cd %s ; git fetch http://yauniks.dynvpn.de/%s refs/changes/%s/%s/%s' % (project, data['project'], junk, number, patch_count))
